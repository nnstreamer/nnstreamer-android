...
We provide example llama2.c [configuration file](/documentation/example_conf/llama2c.conf).
To use it, copy the configuration file to assets repository.
...
To build application with llama2.c, you need to enable llama2c option in the [gradle.properties](/gradle.properties).
...
The MLAgent can handle the ML Computation from the other package. The following code should be added to the other package's `AndroidManifest`
